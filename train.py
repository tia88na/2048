...
def train_agent(episodes=1000, save_interval=100, verbose=True):
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % save_interval == 0:
            agent.save_model(f'/home/claude/twentyrl_arena/agent_checkpoint.pkl')
    ...
    # Save final model
    agent.save_model('/home/claude/twentyrl_arena/agent_final.pkl')

    # Save training history
    history_json = {k: [float(v) for v in vals]
                    for k, vals in agent.training_history.items()}
    with open('/home/claude/twentyrl_arena/training_history.json', 'w') as f:
